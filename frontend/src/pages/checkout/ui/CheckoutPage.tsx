...
import { useCheckoutFromCart } from "@/entities/order/api/useOrders"
...
export const CheckoutPage = () => {
  ...
  if (!authUser?.deliveryAddress?.trim()) {
    blockers.push("Укажите адрес дома в разделе «Доставка»")
  }
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      navigate(routes.user.paymentForOrder(order.id), {
        state: {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      {submitError ? (
        <AlertBanner variant="warning" title="Не удалось оформить заказ">
          {submitError}
        </AlertBanner>
      ) : null}
      ...
      {procurement ? (
        <Card>
          ...
          {procurement.driverName ? (
            <p className="mt-1 text-xs text-slate-500">
              Водитель: {procurement.driverName}
      ...
      <Card>
        ...
        <p className="mt-2 text-xs text-slate-500">
          Водитель привезёт заказ по адресу дома в посёлке
        </p>
      </Card>
