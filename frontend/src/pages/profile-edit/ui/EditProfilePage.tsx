...
export const EditProfilePage = () => {
  ...
  const { register, handleSubmit, reset, formState: { isSubmitting } } = useForm<EditProfileForm>()
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 p-4 pb-8">
      ...
      <Card className="flex flex-col gap-3">
        <label className="text-sm font-medium text-slate-700">
          ФИО
          <Input className="mt-1" {...register("fullName", { required: true })} />
        </label>
        <label className="text-sm font-medium text-slate-700">
          Телефон
          <Input className="mt-1" type="tel" {...register("phone")} />
        </label>
        {user?.email ? (
