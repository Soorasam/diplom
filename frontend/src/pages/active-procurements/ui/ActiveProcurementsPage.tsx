...
export const ActiveProcurementsPage = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (userLocationId) {
      list = list.filter((p) => isProcurementEligibleForUser(p, user))
    }
    ...
    return list
  }, [procurements, deliveryFilter, sort, user, userLocationId])
  ...
  return (
    <PageShell>
      ...
      {isLoading ? (
        ...
        viewMode === "map" ? (
          ...
          <ul className="flex flex-col gap-4">
            {filtered.map((p) => {
              ...
              const notInRoute = userLocationId
                ? !isProcurementEligibleForUser(p, user)
                : true
              return (
                <li key={p.id}>
        ...
        <EmptyState
          icon={Package}
          title={userLocationId ? "Сборов для вашего посёлка пока нет" : "Сборов пока нет"}
          description={
            userLocationId
